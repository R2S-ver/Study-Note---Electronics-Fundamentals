...
#### 🟩2.2. 电压的本质：电场力的空间累积
物理学中，电压的本质是**电势差**。
电池或者电源的作用，是通过化学能或磁能，将正负电荷强行分开，在空间中制造出一个**电场**。当电子在电场中移动时，电场力会对它做功。电压就是单位电荷在两点之间移动时，电场力所做的功：
...
**场景分析：** 一个 12V 电池连接一个 $100\Omega$ 的电阻 ($R_1$)，后面跟着两个并联的 $200\Omega$ 电阻 ($R_2, R_3$)。
1. **应用 KVL（能量视角）：** 电子离开电池时携带了 12V 的电势能。它在经过 $R_1$ 时必须消耗一部分，剩下的在经过并联区域（$R_2$ 或 $R_3$ 任意一条路径）时全部消耗完毕。因此，$V_{R1} + V_{\text{parallel}} = 12\text{V}$。
2. **应用 KCL（电荷守恒视角）：** 从 $R_1$ 流出的电子大军来到了分叉节点。由于 $R_2$ 和 $R_3$ 的阻力（阻值）一模一样，且节点不能堆积电荷，电子流只能完美地一分为二，各自通过一半。
